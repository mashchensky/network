...
:inetRouter => {
  ...
  :centralRouter => {
        ...
        :net => [
                   ...
                   {ip: '192.168.253.1', adapter: 6, netmask: "255.255.255.252", virtualbox__intnet: "o2-net"},
                   {ip: '192.168.254.1', adapter: 7, netmask: "255.255.255.252", virtualbox__intnet: "o1-net"},
                ]
  },
  ...
  :office2Router => {
        ...
        :net => [
                   ...
                   {ip: '192.168.2.193', adapter: 5, netmask: "255.255.255.192", virtualbox__intnet: "hw2-net"}
                ]
  },
    ...
    config.vm.define boxname do |box|
        ...
        when "centralRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            echo "192.168.2.0/24 via 192.168.254.2 dev eth6" > /etc/sysconfig/network-scripts/route-eth6
            echo "192.168.1.0/24 via 192.168.253.2 dev eth5" > /etc/sysconfig/network-scripts/route-eth5
            systemctl restart network
            systemctl restart network
            sysctl net.ipv4.conf.all.forwarding=1
            SHELL
